...
def metadata_parser(config):
    ...
    if end_date is None:
        ...
        if datetime.datetime.now() > datetime.datetime.strptime(end_date, config.timestampformat):
            end_date = datetime.datetime(2099,12,31,23,59,59).strftime(config.timestampformat)
