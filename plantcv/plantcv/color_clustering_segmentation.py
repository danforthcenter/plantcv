...
def color_clustering_segmentation(img,
                                  project_name="PlantCV",
                                  alias_file="",
                                  algorithm="Gaussian"):
    """
    img: An rgb image
    project_name: This name is used to load the Guassian
                  or Mini-Batch Kmeans model.
    alias_file: A file of a list of pixel color values to
                retreive particular clusters.  Default is
                blank (in which case, clusters are simply
    ...
    gmm = pickle.load(open(os.path.join(params.debug_outdir,
                       str(project_name) + "_" + algorithm +
                       "_Model.mdl"), "rb"))
    colors = pickle.load(open(os.path.join(params.debug_outdir,
                       str(project_name) + "_" + algorithm +
                       "_colors.mdl"), "rb"))
    num_components = pickle.load(open(os.path.join(params.debug_outdir,
                       str(project_name) + "_" + algorithm +
                       "_numberOfComponents.mdl"), "rb"))
    remove = pickle.load(open(os.path.join(params.debug_outdir,
                       str(project_name) + "_" + algorithm +
                       "_removed.mdl"), "rb"))
    remove_grays = pickle.load(open(os.path.join(params.debug_outdir,
                       str(project_name) + "_" + algorithm +
                       "_removed_grays.mdl"), "rb"))
    ...
    if not alias_file == "":
        ...
        if params.debug == 'print':
            ...
            for c in mask_alias.keys():
                pcv.print_image(mask_alias[c],
                                os.path.join(params.debug_outdir,
                                project_name + "_" + algorithm +
                                "_" + str(params.device) + str(c)
                                + ".png"))
        ...
        if params.debug == 'print':
            ...
            for c in range(0, num_components):
                pcv.print_image(sub_mask[c],
                            os.path.join(params.debug_outdir,
                            project_name + "_" + algorithm +
                            "_" + str(params.device) + "_submask_"
                            + str(c) + ".png"))
