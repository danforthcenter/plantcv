...
def read_plantexplorer(filename):
    """
    Read datacubes from PhenoVation B.V. PlantExplorer into a PSII_Data instance.

    Inputs:
        filename        = PlantExplorer .INF filename
    ...
    # GFP and RFP fluorescence intensity data
...
def _frame_contrast(frame):
    """Robust contrast metric for picking 'signal' vs 'dark' frames."""
    f = frame.astype(np.float32)
    p50 = np.percentile(f, 50)
    p99 = np.percentile(f, 99)
    return float(p99 - p50)


def _label_reporter_frames(img_cube, channel_name):
    """
    Create stable frame labels for GFP/RFP cubes.

    Strategy:
      - Identify the most 'uniform' frame as Fdark (lowest contrast)
      - Remaining frames are ordered by contrast (highest = primary signal)
    """
    n_frames = img_cube.shape[2]
    contrasts = [_frame_contrast(img_cube[:, :, i]) for i in range(n_frames)]

    dark_i = int(np.argmin(contrasts))
    remaining = [i for i in range(n_frames) if i != dark_i]
    remaining_sorted = sorted(remaining, key=lambda i: contrasts[i], reverse=True)

    labels = [None] * n_frames
    labels[dark_i] = "Fdark"

    if len(remaining_sorted) >= 1:
        labels[remaining_sorted[0]] = channel_name  # "GFP" or "RFP"

    # Any extra frames get numbered labels
    for j, idx in enumerate(remaining_sorted[1:], start=2):
        labels[idx] = f"{channel_name}{j}"
...
def _process_gfp_data(ps, metadata):
    ...
    Stores raw frames as ps.gfp with dims (x, y, frame_label).
    """
    bin_filepath = _dat_filepath(dataset="GFP", datapath=ps.datapath, filename=ps.filename)
    if os.path.exists(bin_filepath) and metadata.get("GfpMeasDone", "1") != "0":
        img_cube, _, _ = _read_dat_file(dataset="GFP", filename=bin_filepath,
                                        height=int(metadata["ImageRows"]),
                                        width=int(metadata["ImageCols"]))

        frame_labels = ["Fdark", "GFP", "GFP2"]
        ...
        gfp.attrs["long_name"] = "GFP fluorescence intensity measurements"
...
def _process_rfp_data(ps, metadata):
    ...
    Stores raw frames as ps.rfp with dims (x, y, frame_label).
    """
    bin_filepath = _dat_filepath(dataset="RFP", datapath=ps.datapath, filename=ps.filename)
    if os.path.exists(bin_filepath) and metadata.get("RfpMeasDone", "1") != "0":
        img_cube, _, _ = _read_dat_file(dataset="RFP", filename=bin_filepath,
                                        height=int(metadata["ImageRows"]),
                                        width=int(metadata["ImageCols"]))
        ...
        rfp.attrs["long_name"] = "RFP fluorescence intensity measurements"
