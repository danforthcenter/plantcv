...
def read_phenovation(filename):
    """
    Read datacubes from PhenoVation B.V. (such as CropReporter or PlantExplorer) into a PSII_Data instance.
    ...
    # Time (dark-adapted, light-adapted, and second dark-adapted) PAM measurements
...
def _process_pml_data(ps, metadata):
    ...
    if os.path.exists(bin_filepath):
        img_cube, frame_labels, frame_nums = _read_dat_file(dataset="PML", filename=bin_filepath,
                                                            height=int(metadata["ImageRows"]),
                                                            width=int(metadata["ImageCols"]))
        frame_labels = ["Flight", "Fsp", "Fmp", "Flightsat"]
        pml = xr.DataArray(
            data=img_cube[..., None],
            dims=('x', 'y', 'frame_label', 'measurement'),
            coords={'frame_label': frame_labels,
...
def _process_pmt_data(ps, metadata):
    ...
    n_fvfm = int(metadata.get("TmPamMeasFvfm", 0)) - 1  # per your requirement
    ...
    # final frame is always F0p (last measurement)
