...
def colorize_label_img(label_img):
    """ Color a label image
    Inputs:
        label_img = 2d image with int values at every pixel, where the values represent for the class the pixel belongs to
    Outputs:
    ...
    _debug(visual=colored_img, filename=os.path.join(params.debug_outdir, str(params.device) + "_colorized_label_img.png"))
