"""Fluorescence Analysis (NPQ component parameter)."""
import os
import numpy as np
import pandas as pd
import xarray as xr
...
def npq_components(ps_da, labeled_mask, n_labels=1, min_bin=0, max_bin="auto",
    ...
    Inputs:
    ps_da        = Photosynthesis xarray DataArray that contains both light and dark adapted measurements from PAM time protocol.
    ...
    measurement_labels = labels for each measurement in ps_da, modifies the variable name of observations recorded
    label              = Optional label parameter, modifies the variable name of
                         observations recorded (default = pcv.params.sample_label).

    Returns:
    qE_global          = DataArray of qE values
    qE_chart           = Histograms of qE estimates
    qI_global          = DataArray of qI values
    qI_chart           = Histograms of qI estimates
    qP_global          = DataArray of qP values
    qP_chart           = Histograms of qP estimates
    qN_global          = DataArray of qN values
    qN_chart           = Histograms of qN estimates
    qL_global          = DataArray of qL values
    qL_chart           = Histograms of qL estimates
    ...
    :param label: str
    :return qE_global: xarray.core.dataarray.DataArray
    :return qE_chart: altair.vegalite.v4.api.FacetChart
    :return qI_global: xarray.core.dataarray.DataArray
    :return qI_chart: altair.vegalite.v4.api.FacetChart
    :return qP_global: xarray.core.dataarray.DataArray
    :return qP_chart: altair.vegalite.v4.api.FacetChart
    :return qN_global: xarray.core.dataarray.DataArray
    :return qN_chart: altair.vegalite.v4.api.FacetChart
    :return qL_global: xarray.core.dataarray.DataArray
    :return qL_chart: altair.vegalite.v4.api.FacetChart
    """
    # Set labels
    labels = _set_labels(label, n_labels)

    if labeled_mask.shape != ps_da.shape[:2] or labeled_mask.shape != ps_da.shape[:2]:
    ...
    if (measurement_labels is not None) and (len(measurement_labels) != ps_da.coords['measurement'].shape[0]):
        fatal_error('measurement_labels must be the same length as the number of measurements in `ps_da`')

    if ps_da.name.lower() != 'ojip_light' or ps_da.name.lower() != 'ojip_dark':
        fatal_error(f"ps_da and ps_da_dark must be DataArrays with names 'ojip_light' and 'ojip_dark', "
                    f"respectively. Instead the inputs are: "
                    f"ps_da: {ps_da.name}, ps_da_dark: {ps_da_dark.name}"
                    )

    # Make an zeroed array of the same shape as the input DataArray
    npq_global = xr.zeros_like(ps_da, dtype=float)
