...
def yii(ps_da, labeled_mask, n_labels=1, auto_fm=False, measurement_labels=None, label="default"):
    ...
    """
    _ = _iterate_analysis(img=ps_da, labeled_mask=labeled_mask, n_labels=n_labels, label=label, function=_analyze_yii,
                          auto_fm=auto_fm, measurement_labels=measurement_labels)
    yii_chart = None
...
def _analyze_yii(img, mask, auto_fm=False, measurement_labels=None, label="default"):
    """
    Calculate and analyze PSII efficiency estimates from fluorescence image data.
    ...
    if mask.shape != img.shape[:2]:
        fatal_error(f"Mask needs to have shape {img.shape[:2]}")

    if len(np.unique(mask)) > 2 or mask.dtype != 'uint8':
        fatal_error("Mask must have dtype uint8 and be binary")

    if (measurement_labels is not None) and (len(measurement_labels) != img.coords['measurement'].shape[0]):
        fatal_error('measurement_labels must be the same length as the number of measurements in the DataArray')

    if auto_fm:
        img = reassign_frame_labels(ps_da=img, mask=mask)

    var = img.name.lower()

    mask = mask[..., None, None]
    if var == 'darkadapted':
        yii0 = img.astype('float').where(mask > 0, other=np.nan)
    ...
    for i, mlabel in enumerate(img.measurement.values):
        if measurement_labels is not None:
            mlabel = measurement_labels[i]
