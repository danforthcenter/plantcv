...
def solidity(bin_img, thresh=0.9):
    ...
    thresh          = Solidity threshold above which a region is kept
    ...
    for obj in obj_measures:
        if obj.solidity > thresh:
            # Convert coord values to int
            discs_mask += np.where(labeled_img == obj.label, 255, 0).astype(np.uint8)

    _debug(visual=discs_mask, filename=os.path.join(params.debug_outdir, f"{params.device}_discs_mask_{thresh*10}.png"))
