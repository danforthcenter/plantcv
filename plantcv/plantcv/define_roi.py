...
def define_roi(img, shape, roi=None, roi_input='default', adjust=False, x_adj=0, y_adj=0,
               w_adj=0, h_adj=0):
    ...
    #  as a plantcv ROI (that can be adjusted below)
    if roi_input == 'rgb':
        ...
        for cnt in roi_contour:
            ...
            if shape == 'rectangle':
                ...
                if params.debug == 'print':
                    print_image(ori_img, os.path.join(params.debug_outdir, str(params.device) + '_roi.png'))
                elif params.debug == 'plot':
                    plot_image(ori_img)
                return rect_contour, hierarchy
            elif shape == 'circle':
                x, y, w, h = cv2.boundingRect(cnt)
                ...
                if h > w:
                    ...
                    if params.debug == 'print':
                        ...
                        plot_image(ori_img)
                    return circle_contour, hierarchy
                else:
                    radius = int(h / 2)
                    cv2.circle(background, center, radius, (255, 255, 255), -1)
                    circle = cv2.cvtColor(background, cv2.COLOR_RGB2GRAY)
                ...
                if w > h:
                    ...
                    if params.debug == 'print':
                        ...
                        plot_image(ori_img)
                    return ellipse_contour, hierarchy
                else:
