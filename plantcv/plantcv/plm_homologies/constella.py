...
def constella(cur_plms, PCstarscape,  group_iter, outfile_prefix, debug):
	...
	for c in np.arange(singleton_no-1, 2, -1):
		...
		for g in group_list:
			...
			if empty_count==2:
				pair_names=cur_plms.iloc[cur_index, 1].values
				...
				if (pair_names[0].split('_')[sanity_check_pos]!=pair_names[1].split('_')[sanity_check_pos]):
					cur_plms.iloc[cur_index, 0] = group_iter
	...
	plm_HCA = dendrogram(plm_links, color_threshold=100, leaf_rotation=90, leaf_font_size=16, labels=np.concatenate(cur_plms.loc[:,['plmname']].values, axis=0))
