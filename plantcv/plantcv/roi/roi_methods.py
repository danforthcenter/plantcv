...
def multiple_roi(img, coord, radius, spacing=None, nrows=None, ncols=None):
    ...
    if (type(coord) == tuple) and ((nrows and ncols) is not None):
        ...
        for i in range(0, nrows):
            # The upper left corner is the y starting coordinate + the ROI offset * the vertical spacing between chips
    ...
    # for roi in rois:
    #     mask = cv2.drawContours(mask, roi[0], -1, (i * 10), -1)
    #     i += 1
    # # Reset debug
    # params.debug = debug
    #
    ...
    # Loop over all the ROIs that were stored
    for roi in rois:
