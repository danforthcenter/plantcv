...
def distance_transform(img, distanceType, maskSize, device, debug=None):
    """Creates an image where for each object pixel, a number is assigned that corresponds to the distance to the nearest background pixel.
    ...
    device += 1
    dist = cv2.distanceTransform(img, distanceType,maskSize)
    norm_image = cv2.normalize(dst, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
