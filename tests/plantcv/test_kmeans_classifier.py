...
def test_kmeans_classifier(test_data):
    ...
    test_labeled, _, _ = read_image(input_dir+"/labeled_image.png")
    test_labeled_gray, _, _ = read_image(input_dir_gray+"/labeled_image_gray.png")
    ...
    for i in range(4):
        assert (read_image(input_dir+"/label_example_"+str(i)+".png")[0] == mask_dict[str(i)]).all()
    for i in range(4):
        assert (read_image(input_dir_gray+"/label_example_gray_"+str(i)+".png")[0] == mask_dict_gray[str(i)]).all()
    ...
    combo_example, _, _ = read_image(input_dir+"/combo_mask_example.png")
    combo_example_gray, _, _ = read_image(input_dir_gray+"/combo_mask_example_gray.png")
