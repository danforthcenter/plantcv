...
def test_roi_objects(mode, exp, test_data):
    ...
    roi_obj = Objects(roi, roi_str)
    _, _, area = roi_objects(img=img, roi_objects=roi_obj, objects=obj, roi_type=mode)
...
def test_roi_objects_bad_input(test_data):
    ...
    with pytest.raises(RuntimeError):
        _ = roi_objects(img=img, roi_type="cut", roi_objects=roi_obj, objects=obj)
...
def test_roi_objects_grayscale_input(test_data):
    ...
    _, _, area = roi_objects(img=img, roi_type="partial", roi_objects=roi_obj, objects=obj)
    # Assert that the contours were filtered as expected
    assert area == 221
...
def test_roi_objects_no_overlap(test_data):
    ...
    roi_str = np.array([[[-1, -1, -1, -1]]], dtype=np.int32)
    roi_obj = Objects(roi, roi_str)
    _, _, area = roi_objects(img=img, roi_objects=roi_obj, objects=obj, roi_type="partial")
    # Assert that the contours were filtered as expected
    assert area == 0
...
def test_roi_objects_nested():
    ...
    roi_str = np.array([[[-1, -1, -1, -1]]], dtype=np.int32)
    roi_obj = Objects(roi, roi_str)
    _, _, area = roi_objects(img=img, roi_objects=roi_obj, objects=obj, roi_type="largest")
    assert area == 580
