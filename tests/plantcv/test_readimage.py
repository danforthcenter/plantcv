...
def test_plantcv_readimage_native(test_data):
    """Test for PlantCV."""
    img, path, img_name = read_image(filename=test_data.small_rgb_img, mode='native')
...
def test_readimage_grayscale(mode, test_data):
    """Test for PlantCV."""
    img, _, _ = read_image(filename=test_data.small_gray_img, mode=mode)
    assert len(img.shape) == 2


def test_readimage_rgb(test_data):
    """Test for PlantCV."""
    img, _, _ = read_image(filename=test_data.small_gray_img, mode="rgb")
...
def test_readimage_rgba(mode, depth, test_data):
    """Test for PlantCV."""
    img, _, _ = read_image(filename=test_data.rgba_img, mode=mode)
    assert img.shape[2] == depth


def test_readimage_csv(test_data):
    """Test for PlantCV."""
    img, _, _ = read_image(filename=test_data.thermal_img, mode="csv")
    assert len(img.shape) == 2


def test_readimage_envi(test_data):
    """Test for PlantCV."""
    hsi = read_image(filename=test_data.envi_bil_file, mode="envi")
    assert hsi.array_data.shape[2] == 978


def test_readimage_nd2(test_data):
    """Test for PlantCV."""
    img, _, _ = read_image(filename=test_data.nd2_img, mode="nd2")
    assert len(img.shape) == 3
...
def test_readimage_bad_file():
    ...
    with pytest.raises(RuntimeError):
        _ = read_image(filename="test.png")
