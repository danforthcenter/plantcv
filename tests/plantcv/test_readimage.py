...
def test_plantcv_readimage_native(test_data):
    img, path, img_name = readimage(filename=test_data.camelina_rgb_img, mode='native')
    expected = [3] + list(os.path.split(test_data.camelina_rgb_img))
...
def test_readimage_grayscale(mode, test_data):
    img, _, _ = readimage(filename=test_data.camelina_gray_img, mode=mode)
    assert len(img.shape) == 2


def test_readimage_rgb(test_data):
    img, _, _ = readimage(filename=test_data.camelina_gray_img, mode="rgb")
    assert len(img.shape) == 3


@pytest.mark.parametrize("mode,depth", [["rgba", 4], ["native", 3]])
def test_readimage_rgba(mode, depth, test_data):
    img, _, _ = readimage(filename=test_data.owl_rgba_img, mode=mode)
