...
* **group_name** (str, default = `"auto"`): either a metadata term (e.g. `"imgtype"`) used to create a unique name for each image in an
...
* **append**: (bool, default = `False`): if `True` will append results to an existing json file. If `False`, will delete
